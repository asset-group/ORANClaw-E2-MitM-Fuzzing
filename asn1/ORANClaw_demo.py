        ...
        for field in asn1_fields:
            ...
            for target_field in fields_to_mutate:
                ...
                if random_value is not None:
                    ...
                    print(Fore.RED + f"[+] Mutated field '{target_field}' to {random_value} in {decoded['definition']}")
...
# Connect to RIC with retry if disconnected
                ...
                while True:
                    ...
                    container_name = "oran-orchestration-xapp-kpm-monitor-1"

                    # Step 1: Ensure xApp container is running
                    ensure_xapp_running(container_name)

                    # Step 2: Start the Docker container restart in a background thread
                    threading.Thread(
                        target=restart_container_periodically,
                        args=(container_name,),
                        daemon=True
                    ).start()
    ...
    def duplicate_packet(self, msg, duplication_count=1, delay_ms=0):
        """
        Duplicate and send packets with optional delay.
        
        Args:
            msg: Original message to duplicate
            duplication_count: Number of duplicates to send (default: 1)
            delay_ms: Delay between duplicates in milliseconds (default: 0)
        """
        try:
            # Send original packet first
            self.ric_client.sctp_send(msg, ppid=socket.htonl(0))
            print(Fore.GREEN + f"[+] Sent original packet")
            ...
            for i in range(duplication_count):
                ...
                self.ric_client.sctp_send(msg, ppid=socket.htonl(0))
                print(Fore.YELLOW + f"[+] Sent duplicate #{i+1}")
    ...
    def proxy_traffic(self):
        ...
        while True:
            try:
                    ...
                    if sock == self.conn_from_xapp:
                        try:
                            ...
                            if not modified_msg:
                                ...
                                try:
                                    ...
                                    ### Try duplication here##?
                                    
                                    # if self.duplication_enabled and random.random() < self.duplication_probability:
                                    #     duplicate_count = random.randint(1, self.max_duplicates)
                                    #     delay = random.randint(0, 5)  # 0-100ms delay between duplicates
                                    #     print(Fore.MAGENTA + f"[FUZZ] Duplicating packet {duplicate_count} times with {delay}ms delay")
                                    #     self.duplicate_packet(modified_msg, duplicate_count, delay)
                                    # else:
                                    #     self.ric_client.sctp_send(modified_msg, ppid=socket.htonl(0))
                                    end_time = time.time()
                        ...
                        try:
                            ...
                            try:
                                ...
                                print(Fore.RED + f"[!] SCTP recv failed: {e}")
                                raise
                            ...
                            if not response:
                                ...
                                try:
                                    ...
                                    print(Fore.YELLOW + f"[!] Connection to xApp lost during send. Need to wait for new connection.")
                                    return  # Exit proxy_traffic to wait for new xApp connection
                        except Exception as e:
                            print(Fore.YELLOW + f"[-] Error during RIC->xApp forwarding: {e}")
                            raise  # Re-raise to trigger reconnection
                ...
                print(Fore.YELLOW + f"[-] Connection broken: {e}")
                ...
                while True:
                    try:
                        ...
                        print(Fore.YELLOW + f"[!] Reconnection to RIC failed: {e}. Retrying in 3 seconds...")
                        time.sleep(2)
                continue  # restart main loop
            
            except Exception as e:
                print(Fore.YELLOW + f"[-] Exception in proxy_traffic loop: {e}")
                import traceback
                traceback.print_exc()
                print(Fore.YELLOW + "[*] Attempting to reconnect to RIC...\n")
                try:
    ...
    def _generate_diff(self, current_file):
        ...
        try:
            ...
            #if result.stdout.strip():
            #    print(f"[*] Output:\n{result.stdout}")
                            # Calculate cost after mutations are applied
            # cost_result = self.calculate_fuzzing_cost(
            #     max_mutations=self.max_mutations,
            #     timestamp=self.timestamp,
            # )
            # self.cost_history.append(cost_result)
            # print(f"[COST] Total cost: {cost_result['total_cost']:.2f}")
            # print(f"[COST] State changes: {cost_result['state_changes']}")
            # print(f"[COST] Transitions: {cost_result['total_transitions']}")
            # print(Fore.CYAN + "=" * 50)
            # if self.timer:
    ...
    def calculate_fuzzing_cost(self, target_fields=None, max_mutations=1, 
                               alpha=10.0, beta=0.1, mu=0.5, t=100, timestamp=None):
        """
...
def ensure_xapp_running(container_name):
    client = docker.from_env()
    try:
        container = client.containers.get(container_name)
        if container.status != "running":
            print(Fore.YELLOW + f"[Docker] Container '{container_name}' is not running. Starting it...")
            subprocess.run(["docker", "compose", "--profile", "xapp", "up", "-d"], check=True)
            time.sleep(5)  # Give it time to start
        else:
            print(Fore.GREEN + f"[Docker] Container '{container_name}' is already running.")
    except docker.errors.NotFound:
        print(Fore.YELLOW + f"[Docker] Container '{container_name}' not found. Attempting to start with docker compose...")
        subprocess.run(["docker", "compose", "--profile", "xapp", "up", "-d"], check=True)
        time.sleep(5)
        
def restart_container_periodically(container_name, interval_sec=15):
    client = docker.from_env()
    try:
        while True:
            container = client.containers.get(container_name)
            print(Fore.YELLOW + f"[Docker] Restarting container: {container.name}")
            container.restart()
            #print(Fore.GREEN + f"[Docker] Restarted at {time.strftime('%Y-%m-%d %H:%M:%S')}")
            time.sleep(interval_sec)
    except docker.errors.NotFound:
        print(Fore.YELLOW + f"[Error] Container '{container_name}' not found.")
    except Exception as e:
