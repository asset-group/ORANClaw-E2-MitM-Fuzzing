...
# Constants
IP_MITM = "172.31.118.62"   
IP_RIC = "127.0.0.1"        # Change to localhost
IP_XAPP = "127.0.0.1"
PORT = 36422

#srsRAN
#PORT_RIC = 36422        # Match the E42_Port from RIC config
#PORT_XAPP = 36422 
    ...
    def process_message(self, msg, direction="to_ric"):
        ...
        try:
            ...
            if direction == "to_ric":
                ...
                fuzz_strategy = random.choices(["asn1", "json", "both"], weights=[0.6, 0.3, 0.1], k=1)[0]
...
class SCTPMITMProxy:
    ...
    def __init__(self, mitm_ip=IP_MITM, xapp_ip=IP_XAPP, ric_ip=IP_RIC, port=PORT):
        ...
        self.ric_ip = ric_ip
        self.port = port
        #self.ric_port = ric_port      # Port for RIC connection
        #self.xapp_port = xapp_port   
        self.server = None
    ...
    # Connect to RIC with retry if disconnected
                while True:
                    try:
                        ...
                        #self.ric_client.bind((self.mitm, self.port))

                        #self.ric_client.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
                        
                        #print(Fore.YELLOW + f"[*] Binding RIC client to {self.mitm}ss:{self.port}")
                        print(Fore.YELLOW + f"[*] Connecting to RIC at {self.ric_ip}:{self.port}...")
                        self.ric_client.connect((self.ric_ip, self.port))
                        print(Fore.GREEN + f"[+] Successfully connected to RIC at {self.ric_ip}:{self.port}")
                        break
                    except Exception as e:
    ...
    def proxy_traffic(self):
        ...
        #     DEBUG_MODE = True
        #     if DEBUG_MODE:
        #     # Just forward raw bytes without any processing
        #         data = self.conn_from_xapp.recv(4096)
        #         if data:
        #             self.ric_client.send(data)
            try:
